...
int qtdZeros, linhaNula = 0, colunaNula = 0;
    ...
    for( i = 0; i < tam; i++ ) {
        ...
        for ( j = 0; j < tam; j++ ) {
            ...
            // como a matriz {0, 0} tem apenas duas colunas, o nosso looping for do j terminou; terminamos o nosso for looping com qtdZeros = 2;
            //               {1, 0}
        }
        ...
        // qtdZeros = 2, pois é o valor da nosso teste de mesa acima;
        ...
        // ou seja, {0, 0}  linha [0] > valor do j = 0 >> 0, valor do j = 1 >> 0; TODOS OS VALORES DE J É IGUAL A 0, OU SEJA, VAI SER UMA LINHA TODA NULA
        //          {1, 0}  linha [1] > valor do j = 0 >> 1, valor do j = 1 >> 0; PELO MENOS UM DOS VALORES DE J É DIFERENTE DE 0, OU SEJA, NAO VAI SER UMA LINHA TODA NULA
    ...
    for( i = 0; i < tam; i++ ) {
        ...
        for ( j = 0; j < tam; j++ ) {
            if ( matriz[j][i] == 0) { // aqui vamos fazer uma condição parecida com a da linha
                qtdZeros++;           // única diferença é que vamos trocar a matriz[i][j] por matriz[j][i];
            }                         // desse jeito vamos calcular quantas colunas nulas existem
        }                             // de resto é igual
        // ou seja, {0, 0}  coluna [0] > valor do i = 0 >> 0, valor do i = 1 >> 1; // qtdZeros = 1; NÃO É NULA
        //          {1, 0}  coluna [1] > valor do i = 1 >> 0, valor do j = 1 >> 0; // qtdZeros = 2; É NULA
        if ( qtdZeros == tam ) {
